...
const BLOCK_OFFLOAD_MARKER = 'VSR_BLOCK_OFFLOAD_V5';
...
function assets() {
  return {
    ...
    transformerSource: path.join(REPO, 'src', 'wan_transformer3d_audio_2512.py'),
...
function status() {
  ...
  if (fs.existsSync(a.transformerSource)) { try { transformerSource = fs.readFileSync(a.transformerSource,'utf8'); } catch (_) {} }
  ...
  const blockOffloadReady = runtimeSource.includes(BLOCK_OFFLOAD_MARKER) && pipelineSource.includes(BLOCK_OFFLOAD_MARKER) && transformerSource.includes(BLOCK_OFFLOAD_MARKER);
  ...
  if (!blockOffloadReady) missing.push('EchoMimicV3 block-offload runtime V5');
  return { ok: missing.length === 0, name:'EchoMimicV3 Flash', runtimeRoot:ROOT, missing, running:Boolean(activeChild), lowVramPatched, longAudioPatched, blockOffloadReady };
}
...
async function generate(event, payload={}) {
  ...
  const ready=status();
  if(!ready.ok) return {ok:false,error:'EchoMimicV3 Flash block-offload V5 chưa sẵn sàng. Chạy upgrader V5 trước.',status:ready};
  let image,audio;
  ...
  emit('info',`Owner FINAL viability benchmark: 768x768 · 25 FPS · ${BENCH_FRAMES} frames · transformer block-offload V5 · TeaCache · T5/CLIP/VAE phase-managed. Gate: <90s good, 90-180s marginal, >180s DROP.`);
  const args=[a.script,'--image_path',image,'--audio_path',audio,'--prompt','A natural person is speaking with realistic facial expressions, subtle head movement, blinking and conversational emotion.','--num_inference_steps','8','--config_path',a.config,'--model_name',a.model,'--transformer_path',a.transformer,'--save_path',outputDir,'--wav2vec_model_dir',a.audio,'--sampler_name','Flow_Unipc','--video_length',BENCH_FRAMES,'--guidance_scale','5.0','--audio_guidance_scale','2.0','--audio_scale','1.0','--neg_scale','1.0','--neg_steps','0','--seed','43','--enable_teacache','--teacache_threshold','0.1','--num_skip_start_steps','5','--GPU_memory_mode','block_offload_v5','--ulysses_degree','1','--ring_degree','1','--weight_dtype','bfloat16','--sample_size','768','768','--fps','25','--add_prompt','','--negative_prompt','static face, frozen expression, stiff head, bad mouth, deformed face, jitter','--shift','5.0'];
  return new Promise(resolve=>{
    let stdout='',stderr='',settled=false;
    const child=spawn(PYTHON,args,{cwd:REPO,windowsHide:true,env:{...process.env,PYTHONUTF8:'1',PYTHONIOENCODING:'utf-8',PYTORCH_CUDA_ALLOC_CONF:'expandable_segments:True'}}); activeChild=child;
