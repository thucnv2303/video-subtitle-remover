...
function status() {
  ...
  return { ok: missing.length === 0, name:'EchoMimicV3 Flash', runtimeRoot:ROOT, missing, running:Boolean(activeChild), lowVramPatched, longAudioPatched, mmgpV52Ready };
}
...
async function generate(event, payload={}) {
  ...
  const ready=status();
  if(!ready.ok) return {ok:false,error:'EchoMimicV3 Flash MMGP V5.2 chưa sẵn sàng. Chạy upgrader V5.2 trước.',status:ready};
  let image,audio;
