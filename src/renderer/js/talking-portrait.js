(function () {
  ...
  function bind() {
    ...
    document.getElementById('tp-clear-log')?.addEventListener('click', () => { const logEl = document.getElementById('tp-log'); if (logEl) logEl.innerHTML = '<div class="tp-log-empty">Log đã được xóa.</div>'; });
    document.getElementById('tp-image')?.addEventListener('change', (event) => { const file = event.target.files?.[0]; if (!file) return; state.imagePath = filePath(file); if (state.imageUrl) URL.revokeObjectURL(state.imageUrl); state.imageUrl = URL.createObjectURL(file); document.getElementById('tp-image-preview').innerHTML = `<img src="${state.imageUrl}" alt="Ảnh nhân vật">`; document.getElementById('tp-stage').innerHTML = `<img src="${state.imageUrl}" alt="Avatar preview">`; document.getElementById('tp-stage').style.display = ''; appendLog(`Đã chọn ảnh: ${file.name}`); updateReady(); });
