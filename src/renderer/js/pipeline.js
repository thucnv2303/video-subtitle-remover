(function mountApprovedPipeline1() {
  ...
  const sectionTitle = (number, title, icon) => `
    <div class="p1-section-title">
      <span class="p1-section-number">${number}</span>
      <strong>${title}</strong>
      <span class="p1-section-icon">${icon}</span>
    </div>`;
        ...
        <aside class="p1-config-column">
          ...
          <section class="p1-card p1-voice-card">
            ...
            <label class="p1-toggle-row">
              ...
              <input id="step1-voice-stable" type="checkbox" checked><span class="p1-switch"></span>
            </label>
            <label class="p1-toggle-row">
              <span class="p1-toggle-copy"><strong>Chuẩn hóa âm lượng</strong><small>Giữ mức âm lượng nghe đồng đều</small></span>
              <input id="step1-normalize-volume" type="checkbox" checked><span class="p1-switch"></span>
            </label>
          </section>
        </aside>
        ...
        <main class="p1-center-column">
          <section class="p1-card p1-queue-card">
            <div class="p1-card-head">
              ${sectionTitle('3', 'Job Queue', 'QUEUE')}
            ...
            <div class="p1-queue-footer"><span>Tổng: <strong id="job-count">0 video</strong></span><span id="p1-complete-count">Đã hoàn thành: 0/0</span></div>
        ...
        <aside class="p1-detail-column">
          <section class="p1-card p1-detail-card">
            <div class="p1-detail-heading">
              ${sectionTitle('4', 'Chi tiết Job đang chọn', 'JOB')}
            ...
            <div class="p1-tab-pane" data-p1-pane="audio">
              <div class="p1-audio-workspace"><div id="step1-audio-empty" class="p1-audio-empty">Chưa có audio lồng tiếng</div><audio id="step1-detail-audio" class="p1-detail-audio" controls></audio></div>
          ...
          <section class="p1-card p1-log-card">
            <div class="p1-card-head">
              ${sectionTitle('6', 'Console / Log', 'LOG')}
...
function ensurePipelineNav() {
  ...
  if (settings) nav.insertBefore(item, settings); else nav.appendChild(item);
  ...
  document.querySelectorAll('.nav-item').forEach(link => link.addEventListener('click', () => {
    const settingsPage = link.dataset.page === 'settings';
    document.querySelector('.app-container')?.classList.toggle('pipeline1-shell-active', !settingsPage);
    if (brand) brand.textContent = settingsPage ? (brand.dataset.shortBrand || 'VSR') : 'Video Subtitle Remover';
  }));
}

function bindApprovedBehavior() {
  ...
  const syncSettings = () => {
    ...
    if (provider) provider.value = ['gemini','deepseek','ollama'].includes(currentProvider) ? currentProvider : 'gemini';
    if (model) model.value = localStorage.getItem(`ai_model_${provider?.value || 'gemini'}`) || '';
    populateModels(provider?.value || 'gemini');
    syncVoiceOptions();
    ...
    if (speed && Number.isFinite(savedSpeed) && savedSpeed >= .5 && savedSpeed <= 2) {
  ...
  provider?.addEventListener('change', () => {
    ...
    if (model) model.value = localStorage.getItem(`ai_model_${provider.value}`) || '';
    populateModels(provider.value);
    setAiStatus('Đã chuyển provider. Model được nạp theo cấu hình đã lưu.');
  });
  ...
  document.getElementById('step1-btn-refresh-queue')?.addEventListener('click', () => { window.renderJobList?.(); updateQueueSummary(); });
...
function syncVoiceOptions() {
  ...
  voices.forEach((item, index) => {
    ...
    option.textContent = `Giọng clone - ${item.name || index + 1}`;
...
async function checkAi() {
  ...
  setAiStatus('Đang kiểm tra backend...');
  try { await window.api.health(); setAiStatus('Backend sẵn sàng. Cấu hình AI có thể được sử dụng.', 'success'); }
  catch { setAiStatus('Backend chưa kết nối.', 'error'); }
...
async function previewVoice() {
  ...
  if (selected === 'none') { notifyP1('Hãy chọn một giọng trước khi nghe thử.', 'warn'); return; }

  let refAudio = null;
  if (selected.startsWith('clone:')) {
    try { refAudio = JSON.parse(localStorage.getItem('tts_voices') || '[]')[Number(selected.split(':')[1])]?.audioPath || null; }
    catch { refAudio = null; }
  }
  button.disabled = true;
  button.textContent = 'Đang tạo giọng thử...';
  try {
    const result = await window.api.generateTTS('Xin chào, đây là giọng đọc bạn đang chọn.', refAudio, localStorage.getItem('tts_language') || 'vi', selected);
    ...
    await audio.play();
  } catch (error) { notifyP1(error?.message || 'Không thể nghe thử giọng.', 'error'); }
...
function notifyP1(message, type='info') {
  if (typeof window.showToast === 'function') window.showToast(message, type);
  else console.log(`[Pipeline1] ${message}`);
}

function deleteSelected() {
  const state = window._appState;
  const id = state?.pipeline1SelectedJobId;
  if (!state || !id) { notifyP1('Chưa chọn Job để xóa.', 'warn'); return; }
  const job = state.jobs.find(item => item.id === id);
  if (!job || ['processing','queued'].includes(job.status)) { notifyP1('Không thể xóa Job đang xử lý hoặc đang chờ.', 'warn'); return; }
  ...
  try { window.renderJobDetail1?.(); } catch { /* detail helper in legacy app may be unavailable */ }
  updateQueueSummary();
}
...
function deleteAllIdle() {
  const state = window._appState;
  if (!state) return;
  const locked = state.jobs.filter(item => ['processing','queued'].includes(item.status));
  const removed = state.jobs.length - locked.length;
  state.jobs = locked;
  if (!locked.some(item => item.id === state.activeJobId)) state.activeJobId = null;
  if (!locked.some(item => item.id === state.pipeline1SelectedJobId)) state.pipeline1SelectedJobId = null;
  window.renderJobList?.();
  try { window.renderJobDetail1?.(); } catch { /* legacy helper guard */ }
  updateQueueSummary();
  notifyP1(removed ? `Đã xóa ${removed} Job không chạy.` : 'Không có Job có thể xóa.', removed ? 'success' : 'info');
}

function observeQueue() {
  const list = document.getElementById('step1-job-list');
  if (!list) return;
  const refresh = () => requestAnimationFrame(() => { decorateJobCards(); updateQueueSummary(); syncDetailMeta(); });
...
function decorateJobCards() {
  ...
  cards.forEach((card, index) => {
    ...
    const oldControls = header.querySelector('div:last-child');
    ...
    const slot = header.querySelector('.p1-job-actions');
    if (oldControls && slot) { [...oldControls.querySelectorAll('button')].forEach(button => slot.appendChild(button)); oldControls.remove(); }
...
function syncDetailMeta() {
  ...
  if (status) { status.textContent = selected ? statusLabel(selected.status) : 'Chờ xử lý'; status.dataset.state = selected?.status || 'idle'; }
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  steps.forEach(step => step.addEventListener('click', () => {
    ...
    panes.forEach(current => {
      const active = current.id === 'step-' + stepNum + '-content';
      current.style.display = active ? 'block' : 'none';
      current.classList.toggle('active', active);
    });
    document.querySelector('.app-container')?.classList.toggle('pipeline1-shell-active', stepNum === '1');
