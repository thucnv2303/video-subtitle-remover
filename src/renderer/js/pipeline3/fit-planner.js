...
const VOICE_TAIL_HEADROOM_MS = 250;
...
function fitVoice(videoMs,voiceMs){const targetMs=Math.max(1,videoMs-VOICE_TAIL_HEADROOM_MS),speed=voiceMs/targetMs;if(!inRange(speed,VOICE_MIN,VOICE_MAX))return result('fit_voice',videoMs,voiceMs,1,1,false,`Voice cần ${speed.toFixed(3)}x để chừa ${VOICE_TAIL_HEADROOM_MS}ms cuối video, ngoài giới hạn ${VOICE_MIN.toFixed(2)}–${VOICE_MAX.toFixed(2)}x.`);return result('fit_voice',videoMs,voiceMs,speed,1,true,`Chừa ${VOICE_TAIL_HEADROOM_MS}ms cuối video để tránh cắt âm cuối khi encode/mux.`);}
...
export function planP3Fit(videoDurationMs,voiceDurationMs,requestedMode='auto'){
  ...
  if(Math.abs(ratio-1)<=CLOSE_DELTA){const vp=fitVoice(videoMs,voiceMs);if(vp.ok)return{...vp,mode:'auto',selectedStrategy:'fit_voice',reason:`Đã gần khớp; chỉ chỉnh voice đủ để chừa ${VOICE_TAIL_HEADROOM_MS}ms tail headroom.`};return result('auto',videoMs,voiceMs,1,1,true,'Đã gần khớp; không đổi tốc độ.');}
  ...
  const vp=fitVoice(videoMs,voiceMs);if(vp.ok)return{...vp,mode:'auto',selectedStrategy:'fit_voice',reason:`Auto giữ video, chỉnh voice trong giới hạn an toàn và chừa ${VOICE_TAIL_HEADROOM_MS}ms cuối.`};
...
export const P3_FIT_LIMITS=Object.freeze({voiceMin:VOICE_MIN,voiceMax:VOICE_MAX,videoMin:VIDEO_MIN,videoMax:VIDEO_MAX,closeDelta:CLOSE_DELTA,videoRetimeDelta:VIDEO_RETIME_DELTA,voiceTailHeadroomMs:VOICE_TAIL_HEADROOM_MS});
