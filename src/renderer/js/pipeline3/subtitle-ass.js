...
function wrapWords(line, maxChars) {
  ...
  for (const word of words) {
    ...
    if (current && candidate.length > maxChars) { rows.push(current); current = word; }
...
function assColor(hex, opacityPercent = 100) {
  ...
  const alpha = Math.round((1 - clamp(opacityPercent, 0, 100) / 100) * 255).toString(16).padStart(2, '0');
...
function positionEffectTags(config, x, y) {
  const ms = clamp(config?.effectMs || 180, 80, 1200);
...
function safeStyle(config = {}) {
  return {
    ...
    fontSize: clamp(config.fontSize || 46, 10, 160),
    textColor: assColor(config.textColor, clamp(config.textOpacity ?? 100, 20, 100)),
    ...
    underline: config.underline ? -1 : 0,
    spacing: clamp(config.letterSpacing || 0, -2, 12),
    borderStyle: config.bgEnabled ? 3 : 1,
    outlineWidth: clamp(config.outlineWidth || 0, 0, 12),
    shadow: clamp(config.shadow || 0, 0, 12),
...
function coverGeometry(config, width, height) {
  ...
  const x = Math.round(w * clamp(config?.x || 50, 0, 100) / 100);
  const y = Math.round(h * clamp(config?.y || 82, 0, 100) / 100);
  const coverWidth = Math.max(40, Math.round(w * clamp(config?.coverWidth || 92, 10, 100) / 100));
...
function glowStyle(config, style, alignment) {
  const color = String(config?.glowColor || '#3b82f6');
  const primary = assColor(color, 45);
  const outline = assColor(color, 70);
  const glowOutline = clamp(config?.glowOutline || 3, 0, 8);
  return `Style: Glow,${style.fontFamily},${style.fontSize},${primary},${primary},${outline},&HFF000000,${style.bold},${style.italic},${style.underline},0,100,100,${style.spacing},0,1,${glowOutline},0,${alignment},${style.margin},${style.margin},${style.margin},1`;
}

function glowEvents(events, config, width, x, y, alignment) {
  if (!config?.glowEnabled) return [];
  const blur = clamp(config?.glowBlur || 4, 0, 8);
  const prefix = `{\\an${alignment}${positionEffectTags(config || {}, x, y)}\\blur${blur}}`;
  return events.map(cue => `Dialogue: 1,${assTime(cue.start)},${assTime(cue.end)},Glow,,0,0,0,,${prefix}${cleanAssText(cue.text, config, width)}`);
}
...
export function buildP3Ass(job, config, width, height) {
  ...
  const header = [
    ...
    `Style: P3,${style.fontFamily},${style.fontSize},${style.textColor},${style.textColor},${style.outlineColor},${style.bgColor},${style.bold},${style.italic},${style.underline},0,100,100,${style.spacing},0,${style.borderStyle},${style.outlineWidth},${style.shadow},${alignment},${style.margin},${style.margin},${style.margin},1`,
    glowStyle(config, style, alignment),
  ...
  const haloEvents = glowEvents(events, config, w, x, y, alignment);
  const textEvents = events.map(cue => `Dialogue: 2,${assTime(cue.start)},${assTime(cue.end)},P3,,0,0,0,,${prefix}${cleanAssText(cue.text, config, w)}`);
  return [...header, ...bandEvents, ...haloEvents, ...textEvents].join('\n');
}
...
export function decorateKaraokeAss(sourceAss, job, config, width, height) {
  ...
  const replacement = `Style: Karaoke,${style.fontFamily},${style.fontSize},${style.textColor},${style.textColor},${style.outlineColor},${style.bgColor},${style.bold},${style.italic},${style.underline},0,100,100,${style.spacing},0,${style.borderStyle},${style.outlineWidth},${style.shadow},${alignment},${style.margin},${style.margin},${style.margin},1`;
...
export function updateJobDerivedAss(job, config, width, height) {
  ...
  const useOriginal = Boolean(config?.preserveKaraoke && !config?.glowEnabled && job.p3OriginalKaraokeAss && !job.p3CueEdited && !job.p3AssTimedSrt);
