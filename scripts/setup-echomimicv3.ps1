...
$RuntimeTransform = Join-Path $PSScriptRoot 'echomimicv3-runtime-transform.py'
...
} elseif ($inferSource.Contains('VSR_LOW_VRAM_OFFLOAD_V2') -or $inferSource.Contains('VSR_LONG_AUDIO_V2')) {
...
if not all(os.path.exists(path) for path in wan_required):
    ...
    retry("Wan model download", lambda: snapshot_download(repo_id="alibaba-pai/Wan2.1-Fun-V1.1-1.3B-InP", local_dir=wan_root, max_workers=1))
else:
...
if not os.path.exists(flash_target):
    ...
    downloaded = retry("Flash transformer download", lambda: hf_hub_download(repo_id="BadToBest/EchoMimicV3", filename="echomimicv3-flash-pro/diffusion_pytorch_model.safetensors"))
...
if not os.path.exists(os.path.join(audio_root, "config.json")):
    ...
    retry("Audio encoder download", lambda: ms_snapshot_download("TencentGameMate/chinese-wav2vec2-base", local_dir=audio_root))
else:
