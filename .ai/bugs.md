...
| BUG-017 | Clicking failed P1 Job did not open error popup. | Owner runtime report 2026-08-11 | Prior popup implementation was not actually loaded at runtime; earlier click-order hypothesis was incomplete | ROOT CAUSE REVISED; WIRING FIX `332a3460...` + selector fix `eb9eaf11...` — OWNER RETEST PENDING | RUNTIME VERIFIED + CODE OBSERVED |
| BUG-018 | Failed P1 Job needs explicit restart; retry during active Job must queue behind current processing Job. | Owner requirement + source inspection | Approved UI hid legacy retry affordance; newly added retry adapter was dead because its script was not loaded | ROOT CAUSE REVISED; WIRING FIX `332a3460...` + selector fix `eb9eaf11...` — OWNER RETEST PENDING | OWNER REQUIREMENT + CODE OBSERVED |
| BUG-019 | P1 run-UX revision existed in repository but popup/retry/glow were absent in real app. | Owner exact-head retest `ffd7984...` + `index.html`/`preload.js`/`app.js` inspection | `pipeline1-run-ux.js` had no boot-path loader; additionally `.p1-job-state` selector did not exist in generated Job markup | FIX PUBLISHED `332a3460...` + `eb9eaf11...`; FINAL REVIEW PENDING | RUNTIME VERIFIED + CODE OBSERVED |
