...
| BUG-008 | AI settings has ambiguous combined API key/model field and no model scan/select UX | Owner screenshot | NOT YET VERIFIED | ACTIVE | OWNER OBSERVED |
| BUG-009 | Ollama rewrite contract treats model like API key entry | Code inspection | Ollama model is treated as an API-key entry in the rewrite contract | ACTIVE | CODE OBSERVED |
