...
CORRECT_OWNER_FRAGMENT = "task 34 đã oke"
...
def patch_current_state(data: bytes) -> bytes:
    ...
    data = replace_unique(
        data,
        "Owner evidence: direct Owner report on 2026-08-07: " + MALFORMED_OWNER_FRAGMENT,
        "Owner evidence: direct Owner report on 2026-08-07: " + CORRECT_OWNER_FRAGMENT,
...
def patch_task_current(data: bytes) -> bytes:
    return replace_unique(
        data,
        "- Owner observation: direct Owner report on 2026-08-07: " + MALFORMED_OWNER_FRAGMENT,
        "- Owner observation: direct Owner report on 2026-08-07: " + CORRECT_OWNER_FRAGMENT,
...
def patch_handoff(data: bytes) -> bytes:
    ...
    data = replace_unique(
        data,
        "PASS (direct Owner report on 2026-08-07: " + MALFORMED_OWNER_FRAGMENT + ")",
        "PASS (direct Owner report on 2026-08-07: " + CORRECT_OWNER_FRAGMENT + ")",
...
def main() -> None:
    ...
    for rel, patcher in PATCHERS.items():
        ...
        if MALFORMED_OWNER_FRAGMENT.encode("utf-8") in after:
            fail(f"{rel}: malformed tab+ask owner fragment remains")
        patched[rel] = after
