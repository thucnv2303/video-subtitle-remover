...
def fail(message: str) -> None:
    print(f"STOP — PM VERIFIED CLOSEOUT PRECONDITION FAILED: {message}", file=sys.stderr)
    raise SystemExit(2)
...
def patch_handoff(data: bytes) -> bytes:
    ...
    data = replace_exact(
        ...
        "Project Manager performs the final merge decision for PR #14. No further Anti implementation task or Owner retest is required for Task 034.\r\nMerge remains BLOCKED until explicit PM merge approval.",
        "handoff next permitted action",
        expected_count=2,
    )
    data = replace_exact(
        data,
