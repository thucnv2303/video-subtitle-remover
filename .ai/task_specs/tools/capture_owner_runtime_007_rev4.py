...
OUTPUT = Path(".ai/incidents/RECOVERY-007E-OWNER-RUNTIME-INVENTORY-007-REV4.json")
...
def main() -> int:
    ...
    output_bytes = (json.dumps(evidence, ensure_ascii=False, indent=2) + "\n").encode("utf-8")
    ...
    for line_number, line in enumerate(output_bytes.splitlines(), start=1):
        if line.endswith((b" ", b"\t")):
            raise RuntimeError(f"Generated evidence has trailing whitespace at line {line_number}")
