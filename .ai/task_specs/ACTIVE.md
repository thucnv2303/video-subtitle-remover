...
Execution spec:
`.ai/task_specs/RECOVERY-007E-OWNER-RUNTIME-INVENTORY-007-REV2.md`
...
Anti authorization:
Run only the exact positive-whitelist sequence in the REV2 spec.
...
Anti executes REV2 deterministic inventory. PM reviews exact JSON evidence, then PM creates a separate cleanup/adoption task with an explicit path manifest. No deletion before that review.
