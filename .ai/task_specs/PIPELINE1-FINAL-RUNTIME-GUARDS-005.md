...
- `src/renderer/js/pipelines/pipeline1-ai.js` — exported-audio duration normalization, one bounded script-fit repair, artifact synchronization, and hard final duration gate.
...
- No TTS engine/model/backend architecture change.
- No video tempo stretching to hide an overlong script.
- No truncating/cutting generated speech.
- No unlimited AI/TTS retry loop.
- No dependency churn.

## Verified legacy TTS duration contract relevant to this task
- `/api/tts-retry` currently builds the exported audio timeline as `timed_segments[-1].end + 1000ms`.
- The legacy response field `audio_duration_ms` contains `timed_segments[-1].end`, so it excludes that fixed 1000ms exported silence tail.
- The P1 gate must therefore normalize actual exported duration as `audio_duration_ms + 1000ms` for this contract. If a future response supplies an explicit exact exported-duration field, that field takes precedence.
- This task does not change the backend/TTS generator merely to expose duplicate metadata.
...
2. Use normalized actual exported TTS-track duration, including the known fixed export tail in the current backend contract.
...
10. Log source duration, first exported TTS duration, repair target ratio, second exported TTS duration, and final PASS/FAIL without dumping private payloads.

## Verification required
- `node --check src/renderer/js/pipeline1-run-ux.js`
- `node --check src/renderer/js/pipelines/pipeline1-ai.js`
- deterministic helper tests for duration ratios: 94%, 95%, 100%, 101%, export-tail normalization, and repair-ratio calculation.
...
- source review proves the duration gate compares source duration against normalized exported-track duration rather than the legacy pre-tail field.
...
3. Log must show source duration and normalized exported TTS duration.
